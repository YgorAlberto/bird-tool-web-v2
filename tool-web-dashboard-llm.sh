...
query_shodan_ip() {
    ...
    # 1) Try free InternetDB first
    local result
    result=$(timeout 8 curl -sf "https://internetdb.shodan.io/${ip}" 2>/dev/null)
    ...
    # 2) Fallback to paid Shodan API if key is set
    if [[ -n "$SHODAN_API_KEY" ]]; then
        result=$(timeout 12 curl -sf "https://api.shodan.io/shodan/host/${ip}?key=${SHODAN_API_KEY}&minify=true" 2>/dev/null)
        if [[ $? -eq 0 && -n "$result" ]]; then
            # Convert paid API format to InternetDB-compatible format
            local converted
            converted=$(python3 -c "
import json,sys
try:
    d=json.loads(sys.stdin.read())
    out={'ip':d.get('ip_str',''),'ports':d.get('ports',[]),'cpes':d.get('cpe',[]),'vulns':[],'hostnames':d.get('hostnames',[])}
...
get_llm_analysis() {
    ...
    # Gather ALL data from OUT-WEB-BIRD for deep analysis
    local all_data="${TEMP_DIR}/llm_full_data.txt"
    > "$all_data"

    # ALL subdomains with status and IPs
    echo "=== SUBDOMINIOS (todos) ===" >> "$all_data"
    cat "${TEMP_DIR}/validated_subs.txt" >> "$all_data"
    echo "" >> "$all_data"

    # ALL Shodan/InternetDB data
    echo "=== DADOS SHODAN POR IP ===" >> "$all_data"
    for cache_file in "${SHODAN_CACHE}"/*.json; do
        [[ -f "$cache_file" ]] || continue
        local ip_name=$(basename "$cache_file" .json)
        echo "IP: $ip_name → $(cat "$cache_file" | python3 -c "import json,sys;d=json.load(sys.stdin);print(f'Ports:{d.get(\"ports\",[])} CPEs:{d.get(\"cpes\",[])} Vulns:{d.get(\"vulns\",[])}')" 2>/dev/null)" >> "$all_data"
    done
    echo "" >> "$all_data"

    # ALL sensitive data from CRAFTJS
    echo "=== DADOS SENSIVEIS (BRID-CRAFTJS) ===" >> "$all_data"
    head -100 "$CRAFTJS_FILE" 2>/dev/null >> "$all_data"
    if [[ $total_brid -gt 100 ]]; then
        echo "... (+ $((total_brid - 100)) mais entradas)" >> "$all_data"
    fi
    echo "" >> "$all_data"

    # ALL file contents from OUT-WEB-BIRD (truncated per file)
    echo "=== CONTEUDO DOS ARQUIVOS OUT-WEB-BIRD ===" >> "$all_data"
    for target_dir in "$OUT_DIR"/*/; do
        [[ ! -d "$target_dir" ]] && continue
        local target=$(basename "$target_dir")
        echo "--- Alvo: $target ---" >> "$all_data"
        for file in "$target_dir"/*; do
            [[ ! -f "$file" || ! -s "$file" ]] && continue
            local fname=$(basename "$file")
            local fsize=$(wc -c < "$file" | tr -d ' ')
            echo "[FILE: $fname ($fsize bytes)]" >> "$all_data"
            # Include file content (limit per file to avoid overflow)
            head -c 8000 "$file" 2>/dev/null | tr -d '\000' | head -80 >> "$all_data"
            if [[ $fsize -gt 8000 ]]; then
                echo "... (truncado, arquivo completo: $fsize bytes)" >> "$all_data"
            fi
            echo "" >> "$all_data"
        done
    done
    ...
    # Sample URLs
    echo "=== URLS COLETADAS (amostra) ===" >> "$all_data"
    head -200 "$URLS_FILE" >> "$all_data"
    if [[ $total_urls -gt 200 ]]; then
        echo "... (+ $((total_urls - 200)) mais URLs)" >> "$all_data"
    fi

    # Truncate total data to fit LLM context (keep ~30KB)
    local data_content
    data_content=$(head -c 30000 "$all_data")

    local prompt="Você é um analista de segurança especializado em reconhecimento web. Analise TODOS os dados abaixo coletados durante o reconhecimento dos alvos em escopo e forneça uma análise profunda e detalhada.

ESCOPO: $scope
ESTATÍSTICAS: $active subdomínios ativos, $inactive inativos, $total_urls URLs, $total_brid dados sensíveis em JS

DADOS COMPLETOS DO RECONHECIMENTO:
$data_content

Analise PROFUNDAMENTE todos os dados acima. Considere:
1. Superfície de ataque exposta (portas, serviços, tecnologias)
2. Dados sensíveis encontrados em JavaScript (API keys, tokens, rotas internas)
3. Correlação entre subdomínios, IPs e serviços
4. Vulnerabilidades potenciais (CVEs dos Shodan, versões desatualizadas)
5. Configurações problemáticas (FTP anônimo, painéis admin expostos)
6. Pontos de entrada para testes de intrusão

Responda APENAS em JSON com esta estrutura exata:
{
  \"executive_summary\": \"Resumo executivo detalhado de 5-10 linhas cobrindo toda a análise\",
  \"risk_level\": \"CRITICO|ALTO|MEDIO|BAIXO\",
  \"attack_surface\": \"Descrição da superfície de ataque identificada\",
  \"key_findings\": [\"achado detalhado 1\",\"achado detalhado 2\",\"achado detalhado 3\",\"achado 4\",\"achado 5\"],
  \"vulnerabilities\": [\"vulnerabilidade 1 com detalhes\",\"vulnerabilidade 2\"],
  \"sensitive_data\": [\"dado sensível encontrado 1\",\"dado sensível 2\"],
  \"recommendations\": [\"recomendação prioritária 1\",\"recomendação 2\",\"recomendação 3\",\"recomendação 4\"]
}
Responda APENAS o JSON, sem explicação."

    local json_prompt
    json_prompt=$(python3 -c "import sys,json; print(json.dumps(sys.stdin.read()))" <<< "$prompt" 2>/dev/null)

    local response
    response=$(timeout 300 curl -s "$OLLAMA_URL" --max-time 300 \
        -H "Content-Type: application/json" \
        -d "{\"model\":\"$MODEL\",\"prompt\":$json_prompt,\"stream\":false,\"options\":{\"temperature\":0.3,\"num_predict\":4000,\"num_ctx\":32768}}" 2>/dev/null)

    if [[ $? -eq 0 && -n "$response" ]]; then
        local analysis
        analysis=$(echo "$response" | python3 -c "
import sys,json,re
text=json.load(sys.stdin).get('response','')
...
" 2>/dev/null)
    ...
    echo '{"executive_summary":"Análise de superfície de ataque realizada com sucesso.","risk_level":"MEDIO","attack_surface":"Múltiplos subdomínios e serviços identificados.","key_findings":["Subdomínios e IPs mapeados","URLs identificadas","Correlação DNS-IP estabelecida"],"vulnerabilities":["Verificar CVEs nos serviços expostos"],"sensitive_data":["Dados encontrados em JavaScript"],"recommendations":["Revisar subdomínios inativos","Verificar serviços expostos","Monitorar exposição de dados sensíveis"]}'
}
...
generate_nav() {
    ...
    <nav>
        <div class="container">
            <h1>🦅 Bird Tool Web Analyzer <span class="llm-badge">🤖 IA</span></h1>
            <div class="nav-links">
...
generate_index() {
    local analysis="$1"
    local active=$(grep -c '|active|' "${TEMP_DIR}/validated_subs.txt" 2>/dev/null || echo 0)
    local inactive=$(grep -c '|inactive|' "${TEMP_DIR}/validated_subs.txt" 2>/dev/null || echo 0)
...
" 2>/dev/null)
    ...
    cat > "${LLM_DASHBOARD_DIR}/index.html" <<EOFHTML
<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>Bird Tool Web - LLM Dashboard</title><link rel="stylesheet" href="assets/style.css"></head><body>
$(generate_nav "index")
<div class="container">
    <div class="card hero-card">
        <h2>📊 Dashboard de Segurança <span class="llm-badge">🤖 Análise IA</span></h2>
        <p>Reconhecimento completo com enriquecimento por inteligência artificial</p>
        <p><small style="color:#64748b">Gerado: $(date '+%Y-%m-%d %H:%M') • Modelo: $MODEL$([ -n "$SHODAN_API_KEY" ] && echo " • Shodan: ✅")</small></p>
    </div>
...
generate_subdomains_page() {
    ...
    cat > "${LLM_DASHBOARD_DIR}/subdomains.html" <<EOFHTML
<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>Subdomínios - LLM Dashboard</title><link rel="stylesheet" href="assets/style.css"></head><body>
$(generate_nav "subdomains")
<div class="container">
    <div class="card">
        <h2>🌐 Subdomínios <span class="llm-badge">🤖 IA</span></h2>
...
EOFHTML
    ...
    sort -t'|' -k2,2r -k1,1 "${TEMP_DIR}/validated_subs.txt" | while IFS='|' read -r sub status ips; do
        [[ -z "$sub" ]] && continue
...
" 2>/dev/null)
        ...
        echo "<tr><td>$idx</td><td>$badge</td><td><a href=\"https://$sub\" target=\"_blank\" style=\"color:#60a5fa\"><code>$sub</code></a></td><td class=\"ip-cell\">$ip_html</td><td>$ports_html</td><td class=\"actions-cell\">$actions</td></tr>" >> "${LLM_DASHBOARD_DIR}/subdomains.html"
        idx=$((idx + 1))
    done
...
print('</div></div>')
" >> "${LLM_DASHBOARD_DIR}/subdomains.html"

    echo "</div><script src=\"assets/script.js\"></script></body></html>" >> "${LLM_DASHBOARD_DIR}/subdomains.html"
}
...
print('<title>BRID-CRAFTJS - LLM Dashboard</title><link rel=\"stylesheet\" href=\"assets/style.css\">')
print('<style>')
...
print('</style>')
print('</head><body>')
" > "${LLM_DASHBOARD_DIR}/brid-craftjs.html"

    generate_nav "brid" >> "${LLM_DASHBOARD_DIR}/brid-craftjs.html"
...
print(f'<div class=\"card\"><h2>🔑 BRID-CRAFTJS <span class=\"llm-badge\">🤖 IA</span></h2>')
...
generate_urls_page() {
    ...
    cat > "${LLM_DASHBOARD_DIR}/urls.html" <<EOFHTML
<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>URLs - LLM Dashboard</title><link rel="stylesheet" href="assets/style.css"></head><body>
$(generate_nav "urls")
<div class="container">
    <div class="card"><h2>🔗 URLs Coletadas <span class="llm-badge">🤖 IA</span></h2><p>Total: <strong id="visibleCount">$count</strong> URLs únicas em escopo</p></div>
...
EOFHTML
    ...
    while IFS= read -r url; do
        ...
        echo "<tr><td>$idx</td><td><a href=\"$url\" target=\"_blank\" style=\"color:#60a5fa;word-break:break-all\">$safe</a></td></tr>" >> "${LLM_DASHBOARD_DIR}/urls.html"
        idx=$((idx + 1))
    done < "$sorted_urls"
    echo "</tbody></table></div></div><script src=\"assets/script.js\"></script></body></html>" >> "${LLM_DASHBOARD_DIR}/urls.html"
}
...
" < "$sorted_urls" 2>/dev/null)
    ...
    cat > "${LLM_DASHBOARD_DIR}/tree.html" <<EOFHTML
<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1.0">
<title>Tree - LLM Dashboard</title><link rel="stylesheet" href="assets/style.css">
<style>
...
$(generate_nav "tree")
<div class="container">
    <div class="card"><h2>🌳 Estrutura de Diretórios <span class="llm-badge">🤖 IA</span></h2>
...
main() {
    ...
    # 4. Shodan enrichment (if key exists)
    enrich_with_shodan

    # 5. LLM analysis
    local analysis='{}'
    if timeout 3 curl -sf "http://localhost:11434/api/tags" &>/dev/null 2>&1; then
        log_success "Ollama disponível — gerando análise IA..."
        analysis=$(get_llm_analysis)
        log_success "Análise IA recebida"
    else
        log_warn "Ollama não disponível — usando análise padrão"
        analysis='{"executive_summary":"Análise de superfície de ataque realizada.","risk_level":"MEDIO","key_findings":["Subdomínios mapeados com status","IPs correlacionados","URLs coletadas"],"recommendations":["Revisar subdomínios inativos","Verificar serviços expostos","Monitorar dados sensíveis"]}'
    fi
    ...
    rm -f "${LLM_DASHBOARD_DIR}/all-subdomains.html"
    rm -f "${LLM_DASHBOARD_DIR}/valid-subdomains.html"
    rm -f "${LLM_DASHBOARD_DIR}/fierce.html"
    rm -f "${LLM_DASHBOARD_DIR}/repos.html"

    log_success "Dashboard LLM gerado em: ${LLM_DASHBOARD_DIR}/"
...
DASHBOARD_DIR = os.environ.get("LLM_DASHBOARD_DIR", "dashboard-llm")
...
for f in glob.glob(os.path.join(OUT_DIR, "*", "*-dnsenum")):
    ...
    try:
        with open(f) as fh:
            for line in fh:
                line = line.strip()
                if not line or line.startswith("dnsenum") or line.startswith("-----"): continue
                if line.endswith(":") and not line.startswith(" "): continue
                parts = line.split()
...
for r in dns_records:
    ...
    if rtype == 'TXT' and 'v=spf1' in val.lower():
        if '-all' in val: policy,color,icon = '-all (Hard Fail)','#4ade80','✅'
        elif '~all' in val: policy,color,icon = '~all (Soft Fail)','#fbbf24','⚠️'
        elif '?all' in val: policy,color,icon = '?all (Neutral)','#f87171','🔴'
        elif '+all' in val: policy,color,icon = '+all (Pass All — DANGEROUS!)','#ef4444','🚨'
        else: policy,color,icon = 'No qualifier','#fbbf24','❓'
        spf_findings.append({'domain': dom, 'name': name, 'value': val, 'policy': policy, 'color': color, 'icon': icon})
...
out.write('<title>DNS Analysis - LLM Dashboard</title><link rel="stylesheet" href="assets/style.css">\n')
out.write('<style>\n')
...
out.write('<nav><div class="container"><h1>🦅 Bird Tool Web Analyzer <span class="llm-badge">🤖 IA</span></h1><div class="nav-links">')
out.write('<a href="index.html">Dashboard</a><a href="subdomains.html">Subdominios</a><a href="brid-craftjs.html">BRID-CRAFTJS</a>')
out.write('<a href="urls.html">URLs</a><a href="tree.html">Tree</a><a href="dns.html" class="active">DNS</a>')
out.write('</div></div></nav>\n')

out.write('<div class="container">\n')
out.write(f'<div class="card"><h2>📡 DNS Analysis <span class="llm-badge">🤖 IA</span></h2>')
out.write(f'<p>Registros DNS de <strong>{len(by_domain)}</strong> domínios • <strong>{len(dns_records)}</strong> registros totais</p></div>\n')

if spf_findings:
    out.write('<div class="card"><h3>🛡️ SPF Policy Analysis</h3>\n')
    for spf in spf_findings:
        out.write(f'<div class="spf-card" style="border-color:{spf["color"]}">')
        out.write(f'<strong>{spf["icon"]} {h.escape(spf["domain"])}</strong> — <span style="color:{spf["color"]}">{h.escape(spf["policy"])}</span>')
        out.write(f'<div class="spf-value">{h.escape(spf["value"])}</div></div>\n')
    out.write('</div>\n')
...
if dmarc_findings:
    ...
    for dm in dmarc_findings:
        ...
        out.write(f'<div class="spf-value">{h.escape(dm["value"])}</div></div>\n')
    out.write('</div>\n')
...
for domain in sorted(by_domain.keys()):
    ...
    for rtype in sorted_types:
        ...
        for r in records:
            ...
            if key not in seen: seen.add(key); unique_records.append(r)
        ...
        out.write(f'<span class="dns-arrow">▶</span><span class="dns-type-badge">{h.escape(rtype)}</span>')
        out.write(f'<span style="color:#e2e8f0">{h.escape(rtype)} Records</span><span class="dns-count">{len(unique_records)}</span></div>')
        out.write(f'<div id="{tid}" class="dns-records">')
        for r in unique_records:
            name = r.get('Name',''); addr = r.get('Address',''); target = r.get('Target',''); string = r.get('String',''); port = r.get('Port','')
            d = ''
            if name: d += f'<span class="rec-name">{h.escape(name)}</span> '
            if addr: d += f'→ <span class="rec-val">{h.escape(addr)}</span> '
            if target: d += f'→ <span class="rec-target">{h.escape(target)}</span> '
            if port: d += f':{h.escape(str(port))} '
            if string: d += f'<span style="color:#a78bfa">{h.escape(string[:120])}</span>'
            out.write(f'<div class="dns-record">{d}</div>\n')
        out.write('</div></div>\n')
    out.write('</div></div>\n')
